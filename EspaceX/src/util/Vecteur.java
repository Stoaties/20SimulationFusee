...
/**
 * La classe Vecteur permet de realiser les operations de base sur un vecteur Euclidien en deux dimensions (x,y), ou x et y sont les 
 * composantes du vecteur.
 * 
 * Cette classe est une version 2d modifiee de la classe SVector3d ecrite par Simon Vezina.
 * 
 * @author Simon Vezina
 * @author Caroline Houle
 */
public class Vecteur implements Serializable{//debut classe 
	//champs de base
	...
	/**
	 * Constructeur avec composantes x,y,z
	...
	/**
	 * Methode qui calcule et retourner l'addition du vecteur courant et d'un autre vecteur. Le vecteur courant reste inchange.
	...
	/**
	 * Methode qui calcule et retourne le vecteur resultant de la soustraction d'un vecteur quelconque du vecteur courant. Le vecteur courant reste inchange.
	...
	/**
	 * Methode qui effectue la multiplication du vecteur courant par une scalaire.Le vecteur courant reste inchange.
	...
	public Vecteur normalise() throws Exception
	{
		...
		if(mod < EPSILON) 
		  throw new Exception("Erreur Vecteur: Le vecteur " + this.toString() + " etant nul ou presque nul ne peut pas etre normalise.");
		else
			return new Vecteur(x/mod, y/mod, z/mod);
	}
	...
	public double angleAvecHorizontale() {
		
		if(SMath.nearlyEquals(x, 0)) {
			return 0;
		}
		
		return Math.atan(Math.toRadians(y/x));
	}
	
	public int signeVecteurY() {
